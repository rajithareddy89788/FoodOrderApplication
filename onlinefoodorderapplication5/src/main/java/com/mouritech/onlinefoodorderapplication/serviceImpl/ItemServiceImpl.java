...
import com.mouritech.onlinefoodorderapplication.repository.ItemsRepository2;
...
public class ItemServiceImpl implements ItemService {
	...
	@Autowired
	private ItemsRepository2 itemsRepository2;
	...
	public Items addItems(Long restaurantId, Items items) throws ResourceNotFoundException {
	  ...
	  public  Items findByName(String item_title) {
			
			Optional<Items> result=itemsRepository2.findByItemName(item_title);
		Items items=null;
			if(result.isPresent())
			{
			items=result.get();
			...
			{
				throw new RuntimeException("Did not find product id-"+item_title);
			}
			return items;
		}
